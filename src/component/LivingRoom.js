...
const LivingRoom = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://192.168.1.8:8000/adminpanel/api/homeroom/add', { room: newRooms, name: newNameAppliances, switchname: newSwitchName });
      if (response.status === 201) {
        ...
        window.location.href = 'http://192.168.1.8:8000/adminpanel/api/homeroom'; // Change the URL accordingly
      } else {
  ...
  return (
    <Grid container sx={{backgroundColor: "grey"}}>
      ...
      <Typography sx={{ m: '15px', fontSize: '28px' }}>Living Room</Typography>
