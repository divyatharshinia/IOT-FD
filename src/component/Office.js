...
const Office = ({ username }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://192.168.1.10:8000/adminpanel/api/officeroom/add', { officeroom: newRoomName });
            ...
            <Box sx={{ marginTop: 4 }}>
              {rooms.map((room, index) => (
                ...
                <Button
                  key={index}
                  variant="contained"
                  sx={{ borderRadius: 10, backgroundColor: '#387e8a', marginTop: 2 }}
                >
                  {room}
                </Button>
              ))}
...
const jsonData = [
  { id: 1, roomName: 'All Rooms', description: 'Total 4', icon: <DomainIcon /> },
